...
Step 1 の CLI 実行時、以下の全ステップが内部的に処理されます。

| Step | 実行内容 | 実行者 |
|------|---------|--------|
| 1 | 設定ファイル読み込み（DigestConfig から取得） | CLI内部 |
| 2 | 未処理 Loop 検出（まだらボケ予防） | CLI内部 |
| 3 | ShadowGrandDigest 確認（未確定ファイル数） | CLI内部 |
| 4 | 中間ファイルスキップ検出（欠番検出） | CLI内部 |
| 5 | GrandDigest 確認（各階層の確定済みファイル数） | CLI内部 |
| 6 | 生成可能な階層判定（閾値との比較） | CLI内部 |
| 7 | プレースホルダー検出（まだらボケケース2） | CLI内部 |
| 8 | 推奨アクション提示（生成可能な階層と不足ファイル数） | CLI内部 |

**実行コマンド**: `python -m interfaces.digest_auto --output json`
